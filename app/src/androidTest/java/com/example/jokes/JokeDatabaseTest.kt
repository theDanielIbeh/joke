...
class JokeDatabaseTest {
    ...
    fun insertAndGetJoke() {
        val insertedJoke = Joke(
            id = -1,
            category = "category",
            type = "type",
            joke = "joke",
        ...
        jokeDao.insert(insertedJoke)
        jokeDao.deleteJokeByIdTest(-1)
        val retrievedJoke = jokeDao.getJokeByIdTest(-1)
        assertEquals(null, retrievedJoke)
    }
}
